...
(function () {
  ...
  function createDesktopCardHtml(item, index) {
    ...
    return `
      <a href="${link}" class="tips-card flex flex-col rounded-card bg-white/80 backdrop-blur-[30px] backdrop-filter ${heightClass} 2xl:rounded-card ${bigImageClass} ${paddingClass} transition-all duration-300 cursor-pointer" data-tip-index="${index}">
        ${
          isBigImage
            ? `
  ...
  function createMobileCardHtml(item) {
    ...
    return `
      <a href="${link}" class="tips-card keen-slider__slide min-w-[300px] max-w-[300px] md:min-w-[379px] md:max-w-[379px] transition-all duration-300 cursor-pointer">
