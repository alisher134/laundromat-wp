document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Fetch locations and settings from WordPress API
  try {
    const [apiLocations, settings] = await Promise.all([window.LaundroAPI.getLocations(), window.LaundroAPI.getSettings()]);

    const apiKey = settings?.google_maps_key || 'AIzaSyDL1-M4UBOI_WrsvbEAdCzklSJ_4ZJdB_w';
    if (!settings?.google_maps_key) {
      console.warn('[Location Page] Google Maps API key not found in site settings, using fallback.');
    }
    ...
    if (apiLocations && apiLocations.length > 0) {
      ...
      // Filter out locations with invalid coordinates for the map
      const validLocations = locations.filter((l) => l.geo.lat !== null && l.geo.lng !== null);

      console.log('[Location Page] Loaded', locations.length, 'locations from API. Valid for map:', validLocations.length);
      hideLoadingState();
      initializePage(apiKey);
    } else {
      console.warn('[Location Page] No locations found in API response');
      showErrorState('No locations found.');
      revealHiddenContainers();
    }
  } catch (error) {
    console.error('[Location Page] Error loading locations:', error);
    showErrorState('Failed to load locations. Please check your connection or CORS settings.');
    revealHiddenContainers();
  }

  // --- UI State Functions ---
  function revealHiddenContainers() {
    // Ensure containers are visible even if initialization logic fails
    const hiddenElements = document.querySelectorAll(
      '.opacity-0.translate-y-\\[150px\\], .opacity-0.translate-y-\\[80px\\]',
    );
    hiddenElements.forEach((el) => {
      el.classList.remove('opacity-0', 'translate-y-[150px]', 'translate-y-[100px]', 'translate-y-[80px]');
    });
  }
  ...
  function showLoadingState() {
    ...
    const loadingHtml = `
      <div class="flex flex-col items-center justify-center py-20">
        ...
        <p class="text-text/60 text-lg">Loading locations...</p>
      </div>
    `;

    if (desktopContainer) desktopContainer.innerHTML = loadingHtml;
    if (sliderContainer) sliderContainer.innerHTML = loadingHtml;
    if (mapElement) mapElement.innerHTML = loadingHtml;
  }
  ...
  function showErrorState(message) {
    ...
    if (sliderContainer) sliderContainer.innerHTML = errorHtml;
    if (mapElement) mapElement.innerHTML = errorHtml;
  }

  // --- Initialize Page Function ---
  function initializePage(apiKey) {
    // --- Animation Observer ---
          ...
          controlsContainer.innerHTML = `
             <div aria-label="Slider navigation" class="flex items-center gap-[8px]" role="group">
               <button
                 ...
                 type="button"
               >
                 <svg viewBox="0 0 13 13" fill="none" xmlns="http://www.w3.org/2000/svg" class="text-brand size-[10px] group-hover:text-white transition-colors">
               ...
               <button
                 ...
                 type="button"
               >
                 <svg viewBox="0 0 9 9" fill="none" class="text-brand size-[10px] group-hover:text-white transition-colors">
    ...
    function updateActiveCards() {
      ...
      allCards.forEach((card) => {
        if (parseInt(card.dataset.id) === activeLocationId) {
          card.classList.replace('bg-white', 'bg-[#488EBE]/10');
        } else {
          card.classList.replace('bg-[#488EBE]/10', 'bg-white');
        }
      });
    }
    ...
    function updateMap() {
      ...
      if (activeLoc && activeLoc.geo.lat !== null) {
      ...
      markers.forEach((marker, index) => {
        const isSelected = locations[index].id === activeLocationId;
    ...
    window.initMap = function () {
      ...
      // Use first valid location or default to Athens if none valid
      const validLocations = locations.filter((l) => l.geo.lat !== null);
      const defaultCenter = { lat: 37.9838, lng: 23.7275 }; // Athens
      const center = validLocations.length > 0 ? validLocations[0].geo : defaultCenter;

      console.log('[Location Page] Initializing map with center:', center);
      ...
      const mapOptions = {
        ...
        styles: [
          ...
          { featureType: 'water', elementType: 'geometry.fill', stylers: [{ color: '#87BCE0' }] },
        ],
      };
      ...
      try {
        ...
        console.log('[Location Page] Map initialized successfully with', markers.length, 'markers');
      } catch (e) {
        console.error('[Location Page] Error creating Google Map:', e);
      }
    };
