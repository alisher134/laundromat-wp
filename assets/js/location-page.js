document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    const apiLocations = await window.LaundroAPI.getLocations();
    ...
    if (apiLocations && apiLocations.length > 0) {
      ...
      locations = apiLocations.map((loc) => ({
        id: loc.id,
        key: loc.key,
        title: loc.address || loc.title, // Use address if available, fallback to title
        phone: loc.phone,
      ...
      console.log('[Location Page] Loaded', locations.length, 'locations from API');
      hideLoadingState();
      initializePage();
    } else {
    ...
    showErrorState('Failed to load locations. Please refresh the page.');
  }
