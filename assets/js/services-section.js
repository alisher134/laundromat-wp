(function () {
  ...
  function normalizeCategory(apiCategory, categoryName = null) {
    ...
    try {
      ...
      if (apiCategory.includes('%')) {
        ...
        console.log(
          '[Services Section] normalizeCategory: decoded URL-encoded category',
          apiCategory,
          '->',
          decodedCategory,
        );
    ...
    console.log(
      '[Services Section] normalizeCategory: normalizing',
      apiCategory,
      '->',
      category,
      '(using categoryName:',
    ...
    if (category.includes('laundry') || category.includes('πλύσιμο') || category.includes('plysimo')) {
      console.log('[Services Section] normalizeCategory: ✅ matched laundry');
      return 'laundry';
    }

    console.warn(
      '[Services Section] normalizeCategory: ❌ NO MATCH for category:',
      apiCategory,
      '(decoded:',
      decodedCategory,
      ', categoryName:',
  ...
  async function loadServices() {
    ...
    try {
      ...
      console.log(
        '[Services Section] ALL services from WordPress:',
        services.map((s) => ({
          id: s.id,
          title: s.title,
          category: s.category,
      ...
      console.log(
        '[Services Section] Raw categories from WordPress:',
      ...
      console.log(
        '[Services Section] Services sorted by menu_order:',
        services.map((s) => ({ title: s.title, menu_order: s.menu_order, category: s.category })),
      );
      ...
      services.forEach((service) => {
        ...
        if (normalizedCategory) {
          ...
          if (!servicesByCategory[normalizedCategory]) {
            ...
            console.log(
              `[Services Section] ✅ Mapped API category "${service.category}" (${service.categoryName || 'no name'}) to "${normalizedCategory}" (menu_order: ${service.menu_order})`,
            );
          } else {
          ...
          console.error(
            `[Services Section] ❌ Failed to normalize category "${service.category}" (${service.categoryName || 'no name'}) for service "${service.title}"`,
          );
        }
      });
      ...
      cardOrder.forEach((cat, index) => {
        ...
        if (service) {
          console.log(
            `[Services Section] Card ${index} (${cat}): menu_order=${service.menu_order}, title="${service.title}"`,
          );
        }
      });
      ...
      sortedDesktopCards.forEach((card) => {
        ...
        console.log(
          `[Services Section] Updating desktop card ${cardIndex} -> category "${category}" -> service:`,
          service.title,
          `(menu_order: ${service.menu_order})`,
        );
      ...
      if (serviceSection) {
        ...
        Array.from(mobileCards).forEach((card, index) => {
          ...
          console.log(
            `[Services Section] Updating mobile card ${index} -> category "${category}" -> service:`,
            service.title,
            `(menu_order: ${service.menu_order})`,
          );
          updateCard(card, service);
    ...
    if (descriptionElements.length > 0 && service.description) {
      ...
      if (cleanDescription) {
        descriptionElements.forEach((desc) => {
          ...
          console.log(
            '[Services Section] Updated description:',
  ...
  function initServiceCards() {
    ...
    cards.forEach((card, index) => {
      ...
      imageWrapper.style.transition =
        'width 5500ms cubic-bezier(0.23, 1, 0.32, 1), height 2700ms cubic-bezier(0.23, 1, 0.32, 1)'; // Extremely slow, gradual expansion
